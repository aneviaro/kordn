...
func decodeQueryBody(body []byte, q url.Values, service string, protocol AWSProtocol, c wireCatalog, l DecodeLimits) (string, map[string]Value, url.Values, error) {
	...
	validated := DecodeFailureEvidence{Service: service, Protocol: protocol, ProtocolAvailable: true, ProtocolCertainty: EvidenceAuthoritative, Operation: a, OperationAvailable: true, OperationCertainty: EvidenceValidated}
	if protocol == ProtocolQuery && service == "ec2" {
		return "", nil, nil, NewDecodeFailureError(validated, errors.New("EC2 must use ec2-query protocol"))
	}
	if protocol == ProtocolEC2Query && service != "ec2" {
		return "", nil, nil, NewDecodeFailureError(validated, errors.New("ec2-query is only valid for EC2"))
	}
	...
	for k, x := range all {
		...
		if len(x) != 1 {
			return "", nil, nil, NewDecodeFailureError(validated, errors.New("duplicate query parameter"))
		}
		...
		if strings.HasPrefix(k, "InstanceId.") {
			...
			if er != nil || n < 1 {
				return "", nil, nil, NewDecodeFailureError(validated, errors.New("malformed indexed query parameter"))
			}
			indexed[n] = x[0]
		}
	}
	if len(indexed) > 0 {
		...
		for i, n := range keys {
			if i+1 != n {
				return "", nil, nil, NewDecodeFailureError(validated, errors.New("indexed query parameters are not contiguous"))
			}
